...
def get_event_SF(jets=[], syst="central", algo="CSV", btag_calibrators=btag_calibrators):
    weight = 1.0
    for jet in jets:
        weight *= get_SF(pt=jet.pt, eta=jet.eta, fl=jet.hadronFlavour, val=(jet.btag if algo=="CSV" else jet.btag), syst=syst, algo=algo, wp="", shape_corr=True, btag_calibrators=btag_calibrators)
    return weight
