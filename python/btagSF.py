...
class BtagSF:
    ...
    def __init__(self, csvfile = None, mvafile = None, csvfile_w = None, mvafile_w = None, debug_btagSF = False):
        self.init(csvfile, mvafile , csvfile_w , mvafile_w , debug_btagSF)

    def init(self, csvfile = None, mvafile = None, csvfile_w = None, mvafile_w = None, debug_btagSF = False):

        # load the BTagCalibrationStandalone.cc macro from https://twiki.cern.ch/twiki/bin/view/CMS/BTagCalibration
        csvpath = os.environ['CMSSW_BASE']+"/src/Xbb/python/csv/"
        #"/src/VHbbAnalysis/Heppy/data/csv/"
        ...
        # cMVAv2
        if mvafile:
            calib_cmva = ROOT.BTagCalibration("cmvav2", csvpath+mvafile)
        else:
            calib_cmva = ROOT.BTagCalibration("cmvav2", csvpath+"/cMVAv2_ichep.csv")
        if csvfile_w:
            calib_csv_reweight = ROOT.BTagCalibration("csvv2", csvpath+csvfile_w)
        else:
            calib_csv_reweight = ROOT.BTagCalibration("csvv2", csvpath+"/ttH_BTV_CSVv2_13TeV_2016BCD_12p9_2016_09_7.csv")
        # cMVAv2
        if mvafile_w:
            calib_cmva_reweight = ROOT.BTagCalibration("cmvav2", csvpath+mvafile_w)
        else:
        ...
        #                btag_calibrators[algo+wp[1]+"_"+syst+"_"+fl].load(sf_type_map[algo]["file"], fl_enum[fl], sf_type_map[algo][fl])
                        self.btag_calibrators[algo+wp[1]+"_"+syst+"_"+fl] = ROOT.BTagCalibrationReader(wp[0], syst)
                        for i in [0,1,2] :
                            self.btag_calibrators[algo+wp[1]+"_"+syst+"_"+fl].load(self.sf_type_map[algo]["file"], i,self.sf_type_map[algo][fl])
        ...
        for algo in ["CSV", "CMVAV2"]:
            for syst in ["central", "up_jes", "down_jes", "up_lf", "down_lf", "up_hf", "down_hf", "up_hfstats1", "down_hfstats1", "up_hfstats2", "down_hfstats2", "up_lfstats1", "down_lfstats1", "up_lfstats2", "down_lfstats2", "up_cferr1", "down_cferr1", "up_cferr2", "down_cferr2"]:
                ...
                for i in [0,1,2]:
                  self.btag_calibrators[algo+"_iterative_"+syst].load(self.sf_type_map_reweight[algo]["file"], i , "iterativefit")
    ...
    def get_SF(self, pt=30., eta=0.0, fl=5, val=0.0, syst="central", algo="CSV", wp="M", shape_corr=False):#, btag_calibrators=btag_calibrators):
