...
if __name__ == "__main__":
    ...
    parser.add_option("-x", "--expressions", dest="expressions", default="", help="colon separated")
    ...
    for region in regions:
        ...
        for dataSample in dataSamples:
            ...
            expressions = ":".join(branchList.getListOfBranches()) if len(opts.expressions) < 1 else opts.expressions
            branchList.addCut(expressions)
            sampleTree.enableBranches(branchList.getListOfBranches())

            sampleTree.tree.Scan(expressions, "run==" + opts.run + "&&event==" + opts.event, "colsize=16")
