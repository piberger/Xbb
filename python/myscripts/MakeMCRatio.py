...
#NLO weight 2017
path_ = "/mnt/t3nfs01/data01/shome/gaperrin/VHbb2018/CMSSW_10_1_0/src/Xbb/python/logs_Zll2017/runplot-lo-nlo-iter3/Plots/"
...
#2017
filenameList = [
    'comp_NLOweightCuts_1b_lowpt__Etabb_.root',
    'comp_NLOweightCuts_2b_highpt__Etabb_.root',
    'comp_NLOweightCuts_1b_highpt__Etabb_.root',
    'comp_NLOweightCuts_0b_lowpt__Etabb_.root',
    'comp_NLOweightCuts_2b_lowpt__Etabb_.root',
    'comp_NLOweightCuts_0b_highpt__Etabb_.root']
...
#2017
#filename_ = 'comp_NLOweightCuts_1b_lowpt__Etabb_.root'
#filename_ = 'comp_NLOweightCuts_2b_highpt__Etabb_.root'
#filename_ = 'comp_NLOweightCuts_1b_highpt__Etabb_.root'
#filename_ = 'comp_NLOweightCuts_0b_lowpt__Etabb_.root'
#filename_ = 'comp_NLOweightCuts_2b_lowpt__Etabb_.root'
#filename_ = 'comp_NLOweightCuts_0b_highpt__Etabb_.root'
...
for filename_ in filenameList:
    ...
    for key in ROOT.gDirectory.GetListOfKeys():
        ...
        for h in canvas.GetListOfPrimitives():
            if not isinstance(h, ROOT.THStack): continue
            #print 'list of histograms'
            #h.ls()
            for hi in h.GetHists():
                #print 'name'
                #print hi.GetName()
                #if not 'TFrame' in type(h):continue
                #print 'h name is', h.GetTitle()
                if 'group_NLO' == hi.GetName():
                    hi.SetDirectory(0)
                    H_nlo = hi
                elif 'group_LO' == hi.GetName():
                    hi.SetDirectory(0)
                    H_lo = hi
        ...
        f = ROOT.TF1('f1', '(pol2)', 0, 5)
        #f = ROOT.TF1('f1', '(pol2)*(1-[3]*exp(-[4]*x))', 0, 5)
        #f = ROOT.TF1('f1', '(pol3)*(exp(-[4]*x))', 0, 5)
        #f = ROOT.TF1('f1', '([0]+[1]*x+[2]*x*x)', 0, 5)
        #f = ROOT.TF1('f1', '(pol2)*([3]*exp(-[4]*x))', 0, 5)
        #f = ROOT.TF1('f1', '(pol2)*([3]*exp(-[4]*x))', 0, 5)
        #f = ROOT.TF1('f1', '(pol3)*([4]*exp(-[5]*x))', 0, 5)

        if filename_ == 'comp_NLOweightCuts_1b_highpt__Etabb_.root':
            f = ROOT.TF1('f1', '(pol2)*([3]*exp(-[4]*x))', 0, 5)
