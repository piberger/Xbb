...
dcList = gDirectory.GetListOfKeys()

BDTnameList = ['ZllBDT_lowptCMVA.Nominal','ZllBDT_highptCMVA.Nominal']
#BDTnameList = ['Nominal','Nominal']
#BDTnameList = ['TTW','TTW']

for dc in dcList:
    if not 'BDT' in dc.GetName(): continue
    print 'dc name is', dc.GetName()
    gDirectory.cd(dc.GetName())

    BDTname = ''
    if 'highpt' in dc.GetName():
        BDTname = BDTnameList[0]
    if 'lowpt' in dc.GetName():
        BDTname = BDTnameList[1]

    hist_postfit_sig_bn = gDirectory.Get("total_signal")
    hist_postfit_bkg_bn = gDirectory.Get("total_background")

    print 'hist_postfit_sig_bn name is', hist_postfit_sig_bn.GetName()
    ...
    BDTFormula = TTreeFormula("BDTFormula", BDTname, tree)
    ...
    for ientry in range(nentries):
        ...
        if not getTreeNum:
            BDTFormula.UpdateFormulaLeaves()
            oldTreeNum = treeNum
            getTreeNum = True
        n1 = BDTFormula.GetNdata()
        val = BDTFormula.EvalInstance()
        i += 1
        #print 'val is', val
        #if i == 10: sys.exit()

        #val = getattr(getattr(tree,BDTname),'Nominal')
