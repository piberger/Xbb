...
#include <iostream>
#include <iomanip>
#include "HelperClasses/KinFitter/TKinFitter.h"
#include "HelperClasses/KinFitter/TAbsFitParticle.h"
#include "HelperClasses/KinFitter/TAbsFitConstraint.h"
...
void TKinFitter::addMeasParticle( TAbsFitParticle* particle ) {
  ...
  if ( particle != nullptr ) {
    ...
    std::cout << "Measured particle points to NULL. It will not be added to the KinFitter.";
  }
...
void TKinFitter::addUnmeasParticle( TAbsFitParticle* particle ) {
  ...
  if ( particle != nullptr ) {
    ...
    std::cout << "Unmeasured particle points to NULL. It will not be added to the KinFitter.";
  }
...
Int_t TKinFitter::fit() {
  ...
  do {
    ...
    if( TMath::IsNaN(currF) ) {
      std::cout << "The current value of F is NaN. Fit will be aborted.";
      _status = -10;
    }
    if( TMath::IsNaN(currS) ) {
      std::cout << "The current value of S is NaN. Fit will be aborted.";
      _status = -10;
    }
